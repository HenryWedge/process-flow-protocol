...
class ProcessFlowNode(NetworkNode, ConformanceProtocol, NeighborProtocol):
    ...
    def receive_event(self, event: Event):
        ...
        # self.update_marking(event)
        self.alignment(event)

    def receive_event_2(self, event: Event):
        predecessor_event = self.predecessor_service.find_predecessor_event(
        ...
        self.learn_event(event)
        if predecessor_event:
            self.alignment(predecessor_event)
        self.alignment(event)
